//go:build !ignore_autogenerated
// +build !ignore_autogenerated

/*
Copyright 2023.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by controller-gen. DO NOT EDIT.

package v1alpha1

import (
	runtime "k8s.io/apimachinery/pkg/runtime"
)

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *PVCScalingSpec) DeepCopyInto(out *PVCScalingSpec) {
	*out = *in
	out.Cooldown = in.Cooldown
	out.MaxSize = in.MaxSize.DeepCopy()
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new PVCScalingSpec.
func (in *PVCScalingSpec) DeepCopy() *PVCScalingSpec {
	if in == nil {
		return nil
	}
	out := new(PVCScalingSpec)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *PVCScalingStatus) DeepCopyInto(out *PVCScalingStatus) {
	*out = *in
	out.RequestedSize = in.RequestedSize.DeepCopy()
	in.RequestedAt.DeepCopyInto(&out.RequestedAt)
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new PVCScalingStatus.
func (in *PVCScalingStatus) DeepCopy() *PVCScalingStatus {
	if in == nil {
		return nil
	}
	out := new(PVCScalingStatus)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *PodDiskInfo) DeepCopyInto(out *PodDiskInfo) {
	*out = *in
	if in.PVCScalingStatus != nil {
		in, out := &in.PVCScalingStatus, &out.PVCScalingStatus
		*out = make([]PVCScalingStatus, len(*in))
		for i := range *in {
			(*in)[i].DeepCopyInto(&(*out)[i])
		}
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new PodDiskInfo.
func (in *PodDiskInfo) DeepCopy() *PodDiskInfo {
	if in == nil {
		return nil
	}
	out := new(PodDiskInfo)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *PodDiskInspector) DeepCopyInto(out *PodDiskInspector) {
	*out = *in
	out.TypeMeta = in.TypeMeta
	in.ObjectMeta.DeepCopyInto(&out.ObjectMeta)
	in.Spec.DeepCopyInto(&out.Spec)
	in.Status.DeepCopyInto(&out.Status)
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new PodDiskInspector.
func (in *PodDiskInspector) DeepCopy() *PodDiskInspector {
	if in == nil {
		return nil
	}
	out := new(PodDiskInspector)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyObject is an autogenerated deepcopy function, copying the receiver, creating a new runtime.Object.
func (in *PodDiskInspector) DeepCopyObject() runtime.Object {
	if c := in.DeepCopy(); c != nil {
		return c
	}
	return nil
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *PodDiskInspectorList) DeepCopyInto(out *PodDiskInspectorList) {
	*out = *in
	out.TypeMeta = in.TypeMeta
	in.ListMeta.DeepCopyInto(&out.ListMeta)
	if in.Items != nil {
		in, out := &in.Items, &out.Items
		*out = make([]PodDiskInspector, len(*in))
		for i := range *in {
			(*in)[i].DeepCopyInto(&(*out)[i])
		}
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new PodDiskInspectorList.
func (in *PodDiskInspectorList) DeepCopy() *PodDiskInspectorList {
	if in == nil {
		return nil
	}
	out := new(PodDiskInspectorList)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyObject is an autogenerated deepcopy function, copying the receiver, creating a new runtime.Object.
func (in *PodDiskInspectorList) DeepCopyObject() runtime.Object {
	if c := in.DeepCopy(); c != nil {
		return c
	}
	return nil
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *PodDiskInspectorSpec) DeepCopyInto(out *PodDiskInspectorSpec) {
	*out = *in
	if in.PVCScaling != nil {
		in, out := &in.PVCScaling, &out.PVCScaling
		*out = new(PVCScalingSpec)
		(*in).DeepCopyInto(*out)
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new PodDiskInspectorSpec.
func (in *PodDiskInspectorSpec) DeepCopy() *PodDiskInspectorSpec {
	if in == nil {
		return nil
	}
	out := new(PodDiskInspectorSpec)
	in.DeepCopyInto(out)
	return out
}

// DeepCopyInto is an autogenerated deepcopy function, copying the receiver, writing into out. in must be non-nil.
func (in *PodDiskInspectorStatus) DeepCopyInto(out *PodDiskInspectorStatus) {
	*out = *in
	if in.PodDiskInfo != nil {
		in, out := &in.PodDiskInfo, &out.PodDiskInfo
		*out = make([]PodDiskInfo, len(*in))
		for i := range *in {
			(*in)[i].DeepCopyInto(&(*out)[i])
		}
	}
}

// DeepCopy is an autogenerated deepcopy function, copying the receiver, creating a new PodDiskInspectorStatus.
func (in *PodDiskInspectorStatus) DeepCopy() *PodDiskInspectorStatus {
	if in == nil {
		return nil
	}
	out := new(PodDiskInspectorStatus)
	in.DeepCopyInto(out)
	return out
}
